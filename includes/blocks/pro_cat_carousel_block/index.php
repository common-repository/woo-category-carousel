 ...
 *
 */
function pro_cat_carousel_block_editor_assets()
{	
	global $post; /* get global variables in js from php editor load time. */

	// Enqueue block editor scripts
	wp_register_script(
		'pro-cat-carousel-block-js',
		WOOCC_PLUGIN_URL . '/includes/blocks/pro_cat_carousel_block/pro-cat-carousel-block.js',
		array('wp-blocks', 'wp-components', 'wp-element', 'wp-i18n', 'wp-editor'),
		filemtime(WOOCC_DIR_PATH . '/includes/blocks/pro_cat_carousel_block/pro-cat-carousel-block.js')
	);
	...
	$pro_cat_carousel_block_localize = array(
		'pcc_categories' => $cats,
	);
	wp_localize_script('pro-cat-carousel-block-js', 'pro_cat_carousel_block_localize', $pro_cat_carousel_block_localize); /** do localize js from here. */

	// Enqueue block editor styles
	wp_enqueue_style(
		'pro-cat-carousel-block-css',
		WOOCC_PLUGIN_URL . '/includes/blocks/pro_cat_carousel_block/pro-cat-carousel-block.css',
		filemtime(WOOCC_DIR_PATH . '/includes/blocks/pro_cat_carousel_block/pro-cat-carousel-block.css')
	);

}
add_action('enqueue_block_editor_assets', 'pro_cat_carousel_block_editor_assets');
 ...
 *
 */
function pro_cat_carousel_block_register_block()
{
	if (function_exists('register_block_type')) {
          
		$args = array(
			'editor_script' => 'pro-cat-carousel-block-js',
			'attributes' => array(
				'cats' => array(
			 ...
			 * 
			 */
			'render_callback' => 'pro_cat_carousel_block_render', 
		);
    
		// register dynamic custom block
		register_block_type('wpexpertplugins/pro-cat-carousel', $args);
	}

}
add_action('init', 'pro_cat_carousel_block_register_block');

function remove_blank_cats($items){
     return trim($items) != "";
}
 ...
 *
 */
function pro_cat_carousel_block_render($attributes)
{	
     $attribute_string = "";
     
     if(isset($attributes["cats"]) && !empty($attributes["cats"])){

          $attributes["cats"] = array_filter($attributes["cats"], "remove_blank_cats");
	...
	$content = "[product_catgory_carousel". $attribute_string ."]"; 
     return $content;
}